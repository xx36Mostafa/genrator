...
class Discord:
    async def get_headers(self, payload):
        ...
        _dcfduid = [item for item in cookies if item['name'] == "_dcfduid"][0]["value"]
        _sdcfduid = [item for item in cookies if item['name'] == "_sdcfduid"][0]["value"]
        cookies = f"_dcfduid={dcfduid}; __sdcfduid={_sdcfduid}"
